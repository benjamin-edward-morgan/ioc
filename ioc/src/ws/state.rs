...
pub enum WsInputStateValue {
    Bool { b: bool },
    Float{ f: f64, min: f64, max: f64, step: f64 },
    String{ s: String, max_length: usize },
}

impl From<&WsStateInputConfig> for WsInputStateValue {
    fn from(value: &WsStateInputConfig) -> Self {
        match value {
            WsStateInputConfig::Bool (cfg) => 
                WsInputStateValue::Bool { b: cfg.start },
            WsStateInputConfig::Float (cfg) => {
                if cfg.min > cfg.max {
                    panic!("min must be <= max");
                }
                WsInputStateValue::Float { f: cfg.start, min: cfg.min, max: cfg.max, step: cfg.step }
            },
            WsStateInputConfig::String (cfg)=> 
                WsInputStateValue::String { s: cfg.start.to_string(), max_length: cfg.max_length },
        }
    }
}
    ...
    fn from(value: &WsStateOutputConfig) -> Self {
        match value {
            WsStateOutputConfig::Bool => 
                WsOutputStateValue::Bool { b: None },
            WsStateOutputConfig::Float => 
                WsOutputStateValue::Float { f: None },
            WsStateOutputConfig::String => 
                WsOutputStateValue::String { s: None },
        }
    }
}
            ...
            while let Some(cmd) = cmd_rx.recv().await {
                let result = match cmd {
                    ...
                    WsStateCmd::SetInputs{state} => {
                        let mut updates: HashMap<String, WsInputStateValue> = HashMap::with_capacity(state.len());
                        ...
                        state.iter().for_each(|(key, new_value)| {
                            match (input_state.get_mut(key), new_value) {
                                ...
                                (
                                    Some(WsInputStateValue::Float{ f: current, min, max, step }),
                                    ...
                                    if *current != clamped {
                                        *current = clamped;
                                        updates.insert(
                                            key.to_string(),
                                            WsInputStateValue::Float{ f: clamped, min: *min, max: *max, step: *step }
                                        );
                                    }
                                },

                                (
                                    Some(WsInputStateValue::String{ s: current, max_length }),
                                    WsStateValue::String{ s: new }
                                ) => {
                                    let trimed: String = new.to_string().chars().into_iter().take(*max_length).collect();
                                    if !current.to_string().eq(&trimed) {
                                        *current = trimed.clone();
                                        updates.insert(
                                            key.to_string(),
                                            WsInputStateValue::String{ s: trimed.to_string(), max_length: *max_length }
                                        );
                                    }
                                }
                        ...
                        if updates.len() > 0 {
                            let i_result = i_tx.send(IUpdate{ inputs: updates.clone() }).map_err(|err| {
                                format!("Error sending IUpdate! {:?}", err)
                            });

                            let io_result = io_tx.send(IOUpdate{ inputs: updates, outputs: HashMap::new() }).map_err(|err| {
                                format!("Error sending IOUpdate! {:?}", err)
                            }); 
                            ...
                            match (i_result, io_result) {
                                ...
                                (Err(i_err), Err(io_err)) => Err(format!("i error: {}\nio error: {}", i_err, io_err)),
                            }
                        } else {
                    ...
                    WsStateCmd::SetOutputs { state } => {
                        let mut updates: HashMap<String, WsOutputStateValue> = HashMap::with_capacity(state.len());
                                ...
                                (
                                    ...
                                    match current {
                                        Some(current) => {
                                            if current != new {
                                                *current = *new;
                                                updates.insert(key.to_string(), WsOutputStateValue::Bool { b: Some(*new) });
                                            }
                                        }
                                        None => {
                                            *current = Some(*new);
                                ...
                                (
                                    ...
                                    match current {
                                        Some(current) => {
                                            if current != new {
                                                *current = *new;
                                                updates.insert(key.to_string(), WsOutputStateValue::Float { f: Some(*new) });
                                            }
                                        }
                                        None => {
                                            *current = Some(*new);
                                ...
                                (
                                    ...
                                    match current {
                                        Some(current) => {
                                            if current != new {
                                                *current = (*new).clone();
                                                updates.insert(key.to_string(), WsOutputStateValue::String { s: Some((*new).clone()) });
                                            }
                                        }
                                        None => {
                        ...
                        if updates.len() > 0 {
                            io_tx.send(IOUpdate{ inputs: HashMap::new(), outputs: updates }).map_err(|err| {
                                format!("Error sending IOUpdate! {:?}", err)
                            }).map(|_| ())
                        } else {
                            //no updates to send (set to same value)
