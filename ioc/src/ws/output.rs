        ...
        output_configs.iter().for_each(|(k, v)| {
            match v {
                &WsStateOutputConfig::Bool => {
                    ...
                    outputs.insert(k.to_string(),  WsOutput::Bool { output: output } );
                    ...
                    tokio::spawn(async move {
                        while let Some(b) = rx.recv().await {
                            ...
                            state.insert(k.clone(), WsStateValue::Bool { b: b });
                            let state_cmd = WsStateCmd::SetOutputs { state: state };
                ...
                &WsStateOutputConfig::Float => {
                    let (tx, mut rx) = mpsc::channel(10);
                    let output = WsGenOutput::new(tx);
                    outputs.insert(k.to_string(),  WsOutput::Float { output: output } );
                    ...
                    tokio::spawn(async move {
                        while let Some(f) = rx.recv().await {
                            let mut state = HashMap::with_capacity(1);
                            state.insert(k.clone(), WsStateValue::Float { f: f });
                            let state_cmd = WsStateCmd::SetOutputs { state: state };
                            state_cmd_tx.send(state_cmd).await.unwrap();
                        }
                        info!("ws output done!");
                ...
                &WsStateOutputConfig::String => {
                    let (tx, mut rx) = mpsc::channel(10);
                    let output = WsGenOutput::new(tx);
                    outputs.insert(k.to_string(),  WsOutput::String { output: output } );
                    ...
                    tokio::spawn(async move {
                        while let Some(s) = rx.recv().await {
                            let mut state = HashMap::with_capacity(1);
                            state.insert(k.clone(), WsStateValue::String { s: s });
                            let state_cmd = WsStateCmd::SetOutputs { state: state };
                            state_cmd_tx.send(state_cmd).await.unwrap();
                        }
                    });
