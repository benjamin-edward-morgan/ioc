...
impl DirectController {
    pub fn new<T: Send + Clone + 'static>(mut source: InputSource<T>, sink: OutputSink<T>) -> DirectController {
...
impl DirectControllerConfig {
    pub fn try_build_float(&self, ports: &BoxedPorts) -> Result<JoinHandle<()>, ControllerBuilderError> {
        match (ports.get_float_source(&self.input), ports.get_float_sink(&self.output)) {
            (Ok(input), Ok(output)) => {
            ...
            (input, output) => {
                ...
                if let Err(e) = input { errs.push(e) }
                if let Err(e) = output { errs.push(e) }
                Err(ControllerBuilderError::from_errors(errs))
            }
        }
    }
    pub fn try_build_bool(&self, ports: &BoxedPorts) -> Result<JoinHandle<()>, ControllerBuilderError> {
        match (ports.get_bool_source(&self.input), ports.get_bool_sink(&self.output)) {
            (Ok(input), Ok(output)) => {
                let ctrl = DirectController::new(input, output);
                Ok(ctrl.handle)
            }
    ...
    pub fn try_build_string(&self, ports: &BoxedPorts) -> Result<JoinHandle<()>, ControllerBuilderError> {
        match (ports.get_string_source(&self.input), ports.get_string_sink(&self.output)) {
            (Ok(input), Ok(output)) => {
                let ctrl = DirectController::new(input, output);
                Ok(ctrl.handle)
            }
...
impl TransformController {
    pub fn new<I,O,F>(
        ...
        F: Send + Copy + FnOnce(I) -> O + 'static
      {
        ...
        TransformController { handle: handle }
    }
}