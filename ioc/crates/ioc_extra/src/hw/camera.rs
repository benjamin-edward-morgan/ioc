...
fn start_mjpeg_stream(/*kill_switch: impl Future<Output = ()> + Send + 'static*/
) -> Result<broadcast::Receiver<Option<JpegImage>>, ChildProcessError> {
    let args = [
        "--rotation",
        "180",
        "--width",
...
impl From<Camera> for ModuleIO {
    fn from(cam: Camera) -> Self {
        ModuleIO {
            ...
            outputs: HashMap::from([("enabled".to_owned(), OutputKind::bool(cam.enable))]),
