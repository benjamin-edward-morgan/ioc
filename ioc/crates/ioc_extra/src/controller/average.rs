use ioc_core::{Input, InputSource, Output};
use std::{
    sync::{Arc, Mutex},
...
pub struct WindowedAverageValueController {
    handle: JoinHandle<()>,
}

impl WindowedAverageValueController {
    pub fn new(input: &dyn Input<f64>, output: &dyn Output<f64>, period_ms: u16) -> Self {
        let mut val_avg = ValueAverage::new(input.source());
        let sink = output.sink();
        let handle = tokio::spawn(async move {
            let mut delay = sleep(Duration::from_millis(period_ms.into()));
            loop {
                delay.await;
                if let Err(err) = sink.tx.send(val_avg.read()).await {
                    error!("error sending from windowed avg controller {:?}", err);
                }
