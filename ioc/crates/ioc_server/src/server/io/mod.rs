        ...
        let typed_input = match (config, start) {
            (ServerInputConfig::Float { .. }, ServerInputState::Float { value, .. }) => {
                let input = ServerInput::new(
                    ...
                    *value,
                    subs.update_rx,
                    |state: &ServerInputState| match state {
    ...
    pub(crate) async fn try_build_output(
        ...
        let typed_output = match config {
            ServerOutputConfig::Float => {
                let output =
                    ServerOutput::new(key, self.cmd_tx.clone(), self.channel_size, |f: f64| {
                        ServerOutputState::Float { value: Some(f) }
                    });
                TypedOutput::Float(output)
            }
            ServerOutputConfig::Bool => {
                let output =
                    ServerOutput::new(key, self.cmd_tx.clone(), self.channel_size, |b: bool| {
                        ServerOutputState::Bool { value: Some(b) }
                    });
                TypedOutput::Bool(output)
            }
            ServerOutputConfig::String => {
                let output =
                    ServerOutput::new(key, self.cmd_tx.clone(), self.channel_size, |s: String| {
                        ServerOutputState::String { value: Some(s) }
                    });
                TypedOutput::String(output)
            }
            ServerOutputConfig::Binary => {
                let output =
                    ServerOutput::new(key, self.cmd_tx.clone(), self.channel_size, |b: Vec<u8>| {
                        ServerOutputState::Binary { value: Some(b) }
                    });
                TypedOutput::Binary(output)
            }
            ServerOutputConfig::Array => {
                let output = ServerOutput::new(key, self.cmd_tx.clone(), self.channel_size, |a| {
