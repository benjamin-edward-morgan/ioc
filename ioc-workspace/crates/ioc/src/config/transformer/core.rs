...
use ioc_core::{error::IocBuildError, transformer::{Sum, SumConfig}, InputKind, Transformer, TransformerI};
...
impl TransformerConfig for SumTransformerConfig {
    async fn try_build(&self,  upstream_inputs: &HashMap<String, InputKind>) -> Result<TransformerI, IocBuildError> {
        ...
        self.inputs.iter().for_each(|input_key| {
            match upstream_inputs.get(input_key) {
                Some(InputKind::Float(float)) => inputs.push(float.as_ref()),
                Some(other) => errors.push(
                    IocBuildError::from_string(format!("Expected input '{input_key}' to be a Float but got {other:?}"))
                ),
                None => errors.push(
                    IocBuildError::from_string(format!("No input named '{input_key}'"))
                )
            }
        });
        ...
        Ok(TransformerI{
            ...
            inputs: HashMap::from([
                ("value".to_owned(), InputKind::float(sum.value)),
            ])
        })
    }
