            ...
            loop {
                match device.accel() {
                    Ok(accel) => {
                        ...
                        let vector = vec![Value::Float(vector.0), Value::Float(vector.1), Value::Float(vector.2)];
...
impl From<Lsm303dlhcDevice> for ModuleIO {
    fn from(dev: Lsm303dlhcDevice) -> Self {
        ModuleIO { 
            ...
            inputs: HashMap::from([
                ("accelerometer".to_string(), InputKind::array(dev.accelerometer)),
                ("magnetometer".to_string(), InputKind::array(dev.magnetometer)),
            ]), 
            outputs: HashMap::new()
        }
    }
