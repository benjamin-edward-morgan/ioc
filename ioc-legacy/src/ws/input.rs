...
impl WsInput {
    pub fn from_subscription(subs: ISubscription) -> HashMap<String, WsInput> {
        ...
        subs.start.inputs.iter().for_each(|(k, v)| match v.clone() {
            WsInputStateValue::Bool { b } => {
                let (tx, rx) = watch::channel(b);
                let input = WsInput::Bool {
                    input: WsGenInput::new(rx),
                };
                inputs.insert(k.to_string(), input);
                let input_tx = WsInputTx::Bool { tx };
                input_txs.insert(k.to_string(), input_tx);
            }
            WsInputStateValue::Float { f, .. } => {
                let (tx, rx) = watch::channel(f);
                let input = WsInput::Float {
                    input: WsGenInput::new(rx),
                };
                inputs.insert(k.to_string(), input);
                let input_tx = WsInputTx::Float { tx };
                input_txs.insert(k.to_string(), input_tx);
            }
            WsInputStateValue::String { s, .. } => {
                let (tx, rx) = watch::channel(s);
                let input = WsInput::String {
                    input: WsGenInput::new(rx),
                };
                inputs.insert(k.to_string(), input);
                let input_tx = WsInputTx::String { tx };
                input_txs.insert(k.to_string(), input_tx);
            }
        });
            ...
            while let Ok(update) = subs_receiver.recv().await {
                update.inputs.iter().for_each(|(k, input_state)| {
                    ...
                    let result = match (input_tx, input_state) {
                        (Some(WsInputTx::Bool { tx }), WsInputStateValue::Bool { b }) => {
                            tx.send(*b).map_err(|_| "error sending Bool input update")
                        }
                        (Some(WsInputTx::Float { tx }), WsInputStateValue::Float { f, .. }) => {
                            tx.send(*f).map_err(|_| "error sending Float input update")
                        }
                        (Some(WsInputTx::String { tx }), WsInputStateValue::String { s, .. }) => tx
                            .send(s.to_string())
                            .map_err(|_| "error sending String input update"),
                        (_, _) => {
                            /* noop  */
                            Ok(())
                        }
    ...
    T: Clone + Debug + Sync + Send + 'static,
{
    pub fn new(mut rx: watch::Receiver<T>) -> WsGenInput<T> {
        ...
        bcast_tx
            .send(current_value.clone())
            .expect("failed to broadcast initial value");
        ...
        tokio::spawn(async move {
            loop {
                ...
                bcast_tx
                    .send(update)
                    .expect("failed to broadcast updated value");
